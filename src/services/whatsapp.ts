...
private karyawanData = {
  ...
  private autoVoiceNote: boolean = false;
...
private loadKaryawanData() {
  ...
  private async handleIncomingMessage(msg: any) {
    ...
    // Auto Voice Note (Auto VN) for incoming non-PTT audio if enabled
    if (this.autoVoiceNote && !msg.key.fromMe && messageObj?.audioMessage && !messageObj.audioMessage.ptt) {
      try {
        ...
        if (audioBuffer && audioBuffer.length > 0) {
          ...
          const tmpRaw = `/tmp/autovn_${tmpId}.raw`;
          const tmpOut = `/tmp/autovn_${tmpId}.ogg`;
          fs.writeFileSync(tmpRaw, audioBuffer);
          try {
            execSync(`"${ffmpegPath}" -y -i "${tmpRaw}" -c:a libopus -b:a 48k -vbr on -compression_level 10 -frame_duration 20 -application voip "${tmpOut}"`);
        ...
        console.error("AutoVN incoming audio conversion error:", e);
      }
    }
...
_Catatan: Dikirim sbg dokumen karena terjadi error konversi._` }, { quoted: msg });
    ...
    } else if (body.startsWith(".autovoicenote") || body.startsWith("autovoicenote") || body.startsWith(".autovn") || body.startsWith("autovn")) {
      const rawArg = messageContent.replace(/^\.?(autovoicenote|autovn)\s*/i, "").trim();
      const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
      const isQuotedAudio = quoted?.audioMessage;
      const isQuotedVideo = quoted?.videoMessage;
      const isQuotedDoc = quoted?.documentMessage && (quoted.documentMessage.mimetype?.includes('audio') || quoted.documentMessage.mimetype?.includes('video'));

      // 1. Toggle ON / OFF
      if (rawArg.toLowerCase() === "on" || rawArg.toLowerCase() === "enable" || rawArg.toLowerCase() === "aktif") {
        this.autoVoiceNote = true;
        this.saveBotSettings();
        await this.sock.sendMessage(jid, { 
          text: `🎙️ *Auto Voice Note (Auto VN) Telah Diaktifkan!*\n\nBot sekarang dalam mode Auto VN aktif.` 
        }, { quoted: msg });
        return;
      }
      if (rawArg.toLowerCase() === "off" || rawArg.toLowerCase() === "disable" || rawArg.toLowerCase() === "mati") {
        this.autoVoiceNote = false;
        this.saveBotSettings();
        await this.sock.sendMessage(jid, { 
          text: `🎙️ *Auto Voice Note (Auto VN) Telah Dinonaktifkan.*` 
        }, { quoted: msg });
        return;
      }

      // 2. Reply to media (audio / video / doc) -> Convert to Voice Note (Auto VN)
      if (isQuotedAudio || isQuotedVideo || isQuotedDoc) {
        await this.sock.sendMessage(jid, { text: `⏳ *Sedang memproses media menjadi Voice Note (Auto VN)...*` }, { quoted: msg });
        try {
          ...
          if (!mediaBuffer || mediaBuffer.length === 0) {
            return await this.sock.sendMessage(jid, { text: `❌ Gagal mengunduh media dari pesan yang dibalas.` }, { quoted: msg });
          }

          const tmpId = Date.now() + Math.random().toString(36).substring(2, 7);
          const tmpIn = `/tmp/autovn_${tmpId}_in.raw`;
          const tmpOut = `/tmp/autovn_${tmpId}_out.ogg`;
          fs.writeFileSync(tmpIn, mediaBuffer);

          try {
            execSync(`"${ffmpegPath}" -y -i "${tmpIn}" -c:a libopus -b:a 48k -vbr on -compression_level 10 -frame_duration 20 -application voip "${tmpOut}"`);
            const vnBuffer = fs.readFileSync(tmpOut);
            await this.sock.sendMessage(jid, {
              audio: vnBuffer,
          ...
          console.error("AutoVN media conversion error:", mediaErr);
          await this.sock.sendMessage(jid, { text: `❌ Gagal mengubah media menjadi Voice Note.` }, { quoted: msg });
        }
        return;
      }

      // 3. TTS query: --tts <text> or tts <text>
      if (rawArg.toLowerCase().startsWith("--tts") || rawArg.toLowerCase().startsWith("tts ")) {
        const textToSpeech = rawArg.replace(/^(--tts|tts)\s*/i, "").trim();
        if (!textToSpeech) {
          return await this.sock.sendMessage(jid, { text: `Kirim teks untuk diubah jadi VN!\nContoh: .autovn --tts Halo semuanya apa kabar?` }, { quoted: msg });
        }
        await this.sock.sendMessage(jid, { text: `⏳ *Sedang mengubah teks menjadi Voice Note (Auto VN)...*` }, { quoted: msg });
        try {
          const ttsUrl = `https://translate.googleapis.com/translate_tts?client=gtx&ie=UTF-8&tl=id&q=${encodeURIComponent(textToSpeech)}`;
          const ttsRes = await axios.get(ttsUrl, { responseType: 'arraybuffer' });
          const rawBuffer = Buffer.from(ttsRes.data);

          const tmpId = Date.now() + Math.random().toString(36).substring(2, 7);
          const tmpIn = `/tmp/autovn_tts_${tmpId}.mp3`;
          const tmpOut = `/tmp/autovn_tts_${tmpId}.ogg`;
          ...
          console.error("TTS AutoVN error:", ttsErr);
          await this.sock.sendMessage(jid, { text: `❌ Gagal memproses Voice Note dari teks.` }, { quoted: msg });
        }
        return;
      }

      // 4. If no argument provided: display Help & Status
      if (!rawArg) {
        const helpText = `🎙️ *FITUR AUTO VOICE NOTE (AUTO VN)*\n\nStatus Auto VN: *${this.autoVoiceNote ? '✅ AKTIF (ON)' : '❌ NONAKTIF (OFF)'}*\n\nFitur ini berfungsi untuk mendownload atau mengubah lagu/audio/video menjadi Voice Note (VN / PTT) secara otomatis.\n\n📌 *Cara Penggunaan:*\n│ *1. Cari & Download Musik Jadi VN:*\n│   .autovoicenote <judul lagu>\n│   _Contoh: .autovn bernadya satu bulan_\n│   _Contoh: .autovn dj santuy tiktok_\n│\n│ *2. Dari Link Media / Video:*\n│   .autovoicenote <link youtube/tiktok>\n│   _Contoh: .autovn https://vt.tiktok.com/ZS9pCeuV4/_\n│\n│ *3. Ubah Media Jadi VN:*\n│   Balas (reply) audio/video dengan ketik *.autovn*\n│\n│ *4. Buat VN dari Teks (TTS):*\n│   .autovn --tts <isi pesan>\n│\n│ *5. Saklar Auto VN:*\n│   .autovn on / .autovn off`;
        await this.sock.sendMessage(jid, { text: helpText }, { quoted: msg });
        return;
      }

      // 5. URL or Search Query -> Download audio and convert to Voice Note
      const isUrl = /^(https?:\/\/[^\s]+)$/i.test(rawArg);
      await this.sock.sendMessage(jid, { text: `⏳ *Sedang mencari & memproses audio untuk Auto VN...*` }, { quoted: msg });
      ...
      try {
        ...
        // Check if TikTok
        if (isUrl && rawArg.includes("tiktok.com")) {
          try {
            const fetchRes = await axios.get(`https://www.tikwm.com/api/?url=${rawArg}`);
            if (fetchRes.data && fetchRes.data.code === 0 && fetchRes.data.data.music) {
              audioStreamUrl = fetchRes.data.data.music;
              title = fetchRes.data.data.title || "TikTok Audio";
            }
          } catch(e) {}
        }

        // If not found or YouTube / search query:
        if (!audioStreamUrl) {
          let videoUrl = rawArg;
          if (!isUrl) {
            const search: any = await btch.yts(rawArg);
            if (search.result && search.result.videos && search.result.videos.length > 0) {
              const firstVideo = search.result.videos[0];
              videoUrl = firstVideo.url;
              title = firstVideo.title;
            } else {
              return await this.sock.sendMessage(jid, { text: `❌ Tidak ditemukan lagu/audio untuk "${rawArg}"` }, { quoted: msg });
            }
          }
          ...
          if (ytDownload && ytDownload.download && ytDownload.download.url) {
            audioStreamUrl = ytDownload.download.url;
            if (ytDownload.metadata?.title) title = ytDownload.metadata.title;
          }
        }

        // Direct link fallback
        if (!audioStreamUrl && isUrl) {
          audioStreamUrl = rawArg;
        }

        if (!audioStreamUrl) {
          return await this.sock.sendMessage(jid, { text: `❌ Gagal mengambil audio dari sumber tersebut. Coba judul lagu lain!` }, { quoted: msg });
        }

        const { data } = await axios.get(audioStreamUrl, { 
        ...
        const tmpId = Date.now() + Math.random().toString(36).substring(2, 7);
        const tmpRaw = `/tmp/autovn_dl_${tmpId}.raw`;
        const tmpFixedAudio = `/tmp/autovn_dl_${tmpId}_fixed.ogg`;
        fs.writeFileSync(tmpRaw, buffer);
        ...
        try {
          ...
          console.error("FFmpeg AutoVN conversion error:", convErr);
          await this.sock.sendMessage(jid, { 
            audio: buffer, 
            mimetype: 'audio/mp4', 
            ptt: true 
...
*Sumber*: BMKG`;
      ...
      if (body.startsWith(".joingc") || body.startsWith("joingc")) {
      ...
      } else if (body.startsWith(".createsaluran") || body.startsWith("createsaluran") || body.startsWith(".buatsaluran") || body.startsWith("buatsaluran")) {
         if (!isOwner) return await this.sock.sendMessage(jid, { text: `👑 *Akses Ditolak*\nPerintah ini hanya bisa digunakan oleh Owner!` }, { quoted: msg });
         const rawArgs = messageContent.replace(/^\.?(createsaluran|buatsaluran)\s*/i, "").trim();
         if (!rawArgs) {
             return await this.sock.sendMessage(jid, { 
               text: `📢 *Cara Membuat Saluran WhatsApp (Newsletter)*\n\nFormat: *.createsaluran <Nama Saluran> | <Deskripsi>*\n\nContoh:\n.createsaluran Informasi Bot Batak | Saluran resmi update fitur dan info bot` 
             }, { quoted: msg });
         }
         ...
         if (!name) {
             return await this.sock.sendMessage(jid, { text: `❌ Nama saluran tidak boleh kosong!\nContoh: .createsaluran Saluran Bot Resmi` }, { quoted: msg });
         }
         await this.sock.sendMessage(jid, { text: `⏳ *Sedang membuat saluran WhatsApp "${name}"...*` }, { quoted: msg });
         try {
             const res: any = await this.sock.newsletterCreate(name, description);
             if (res && res.id) {
                 const inviteLink = res.invite ? `https://whatsapp.com/channel/${res.invite}` : "-";
                 const replyMsg = `✅ *SALURAN WHATSAPP BERHASIL DIBUAT!*\n\n📢 *Nama Saluran:* ${res.name || name}\n📝 *Deskripsi:* ${res.description || description || '-'}\n🆔 *JID Saluran:* ${res.id}\n🔗 *Link Undangan:* ${inviteLink}\n\n_Saluran telah terdaftar di WhatsApp dan siap digunakan._`;
                 await this.sock.sendMessage(jid, { text: replyMsg }, { quoted: msg });
             } else {
                 await this.sock.sendMessage(jid, { text: `❌ Gagal membuat saluran. WhatsApp tidak merespons metadata saluran.` }, { quoted: msg });
             }
         } catch(err: any) {
             console.error("Error creating newsletter:", err);
             await this.sock.sendMessage(jid, { text: `❌ Gagal membuat saluran: ${err?.message || "Terjadi kesalahan pada server WhatsApp"}` }, { quoted: msg });
         }
